...
2. **Query Types**:
   - **Trip Planning**: Use a chain of thought to reason through the response. Break it into steps: (1) duration, (2) destinations, (3) activities/attractions. Example: "For a 7-day Italy trip: 1) Plan 7 days. 2) Visit Rome (Colosseum), Florence (Uffizi), Venice (Grand Canal). 3) Try a pasta-making class. Make sure to try to use External Data for attractions (from Geoapify if available) and weather data from External Data for packing suggestions.)"
   - **Packing Suggestions**: Provide a short list (3-5 items) based on destination and weather data from External Data if available. (e.g., "Sunny **Munich**, 20°C: sunscreen, hat"). If weather data is unavailable, use seasonal trends strictly aligned with ${seasonalContext}.
...
`;
    ...
    try {
      ...
      if (
        userInput.includes("Are these attractions") &&
